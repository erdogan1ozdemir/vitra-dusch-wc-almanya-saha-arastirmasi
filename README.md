# VitrA Almanya · Dusch-WC Pazar ve Satış Kanalı Araştırması

Almanya'da Dusch-WC (yıkama fonksiyonlu klozet) kategorisinin arama talebi, rekabet yapısı ve
satış kanalı seçeneklerinin incelendiği saha araştırması. Rapor tarihi: 09.09.2026.

**Yayınlanan rapor:** [`index.html`](index.html) · <https://vitra-dusch-wc-almanya-saha-arastir.vercel.app>

## Kapsam

| Alan | Kapsam |
|---|---|
| Pazar | Almanya (DE), Almanca arama kırılımı |
| İncelenen sayfa | www.vitra-bad.de/entdecken/dusch-wcs/ |
| Arama verisi | 2022-09 - 2026-07 (47 ay) |
| Kelime seti | 172 kelime, 8 komşu kategori |
| Rakip | 6 kategori sayfası, URL düzeyinde organik kelime ve trafik |
| Kanal | 16 kanal koşul matrisi, 15 kanallık dağıtım listesi |
| Veri dosyası | 14 sekmeli Excel, rapora gömülü (70 KB) |

## Rapor bölümleri

**Durum**

1. Yönetici Özeti
2. Almanya Dusch-WC Pazarı ve Kanal Yapısı

**Talep**

3. Kategori Arama Talebi ve Dönemsel Değişim
4. Kategori Adı Değişiyor
5. Retailer Adıyla Yapılan Aramalar

**Rekabet ve konumlandırma**

6. VitrA'nın Mevcut Görünürlüğü ve Kanal Varlığı
7. Rakip Görünürlüğü ve Kelime Kırılımı
8. Akıllı Klozet Almanya'da Nasıl Konumlanmalı
9. Kategori Hangi Markalarla Aranıyor
10. Arama Sonuç Sayfası ve Rekabet Yapısı

**Kanal**

11. Kendi Depodan Dağıtılabilecek Siteler
12. Pazaryeri Koşulları, Ücretleri ve Giriş Şartları
13. Retailer ve Streckengeschäft Kanalı
14. Kendi Kanal ve Google Shopping
15. B2B ve Tesisatçı Kanalı

**Plan**

16. VitrA'nın Satış Yapabileceği Komşu Kategoriler
17. Sonraki Adımlar

**Ek**

18. Yöntem ve Kaynaklar
19. Terim Sözlüğü

## Veri kaynakları

- **Arama hacmi:** Google Keyword Planner (DataForSEO Google Ads API), Almanya (DE), Almanca
- **Sıralama, SERP ve organik trafik:** Ahrefs Site Explorer & SERP Overview, Almanya (DE)
- **Pazar büyüklüğü:** [Unternehmensberatung Titze · Toiletten und Dusch-WCs in D/A/CH bis 2030](https://www.titze-online.com/marktstudien-uebersicht/marktstudie-toiletten-und-dusch-wcs/)
- **Pazaryeri payı:** [HDE Online-Monitor 2026](https://einzelhandel.de/online-monitor)
- **Sanitärkeramik pazarı:** [diy online · Markt für Sanitärkeramik wächst bis 2030 um 16 Prozent](https://www.diyonline.de/d/news/2026/08/05/plus/markt-fuer-sanitaerkeramik-waechst-bis-2030-um-16-prozent.html)
- **Kanal koşulları:** Platformların kendi satıcı sayfaları

Ahrefs ve DataForSEO dışındaki kaynaklar raporun "Yöntem ve Kaynaklar" bölümünde tam adresleriyle
listelenmiştir.

## Yöntem notu

Karşılaştırılan dönemler aynı takvim aylarını kapsar: tam yıl karşılaştırması 2024 ve 2025
takvim yıllarının 12 aylık ortalamasıdır; 2026 için Keyword Planner Temmuz'a kadar veri
döndürdüğünden bu yıl YTD (Ocak-Temmuz) alınmış, 2025 aynı yedi ayla eşlenmiştir.

Google Ads anlamca yakın kelimeler için birleşik hacim döndürür. Aynı hacmi ve aynı aylık
seriyi paylaşan kelimeler raporda tek grup olarak gösterilir; grup üyelerinin hacimleri
toplanmaz.

## Raporun kullanımı

- **Veri dosyası** üstteki ve alttaki "Veri dosyası" düğmesinden indirilir; dosya rapora gömülüdür,
  ayrı bir adrese bağımlı değildir.
- **Tema:** üst bardaki düğme açık ve koyu tema arasında geçiş yapar, tercihi tarayıcıda saklar;
  tercih yoksa işletim sistemi ayarı izlenir.
- **İçindekiler** masaüstünde sol sütunda, mobilde sağ alttaki düğmeyle açılan alt sayfada bulunur.
- **Tablolar** kendi içinde kaydırılır; sütun başlıklarının üzerine gelindiğinde o sütunun hangi
  veriyi taşıdığı açıklanır. Grafiklerde veri noktasına gelindiğinde sayısal değer gösterilir.

## Depo içeriği

Depoya yalnızca yayınlanan rapor ve bu dosya girer. Üretim betikleri, ham veri dosyaları ve
sunum çıktısı `.gitignore` ile dışarıda tutulur.
